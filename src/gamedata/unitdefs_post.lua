...
if (modOptions and modOptions.metalmult) then
  for _,ud in pairs(UnitDefs) do
    ...
    if (em) then
      ud.extractsmetal = em * modOptions.metalmult
    end
  end
end
...
if (modOptions and modOptions.energymult and modOptions.energymult ~= 1) then
  local emult = modOptions.energymult
  for id, ud in pairs(UnitDefs) do
    local energymake = tonumber(ud.energymake)
    local energyuse = tonumber(ud.energyuse)
    if not tobool(ud.canmove) then
      if (energymake and energymake > 20) then -- geo & fusions
        ud.energymake = energymake * emult
      elseif (energyuse and (energyuse > -2.1) and (energyuse < -1.9)) then  -- solars
        ud.energyuse = energyuse * emult
      end
    end
  end
end
...
if (modOptions and modOptions.commtype) then
  ...
  elseif modOptions.commtype == 'cat' then
    for name, ud in pairs(UnitDefs) do
      local unitname = ud.unitname
      if (unitname == "armcom" or unitname == "corcom") then
    ud.maxdamage = 3500
    ud.explodeas = "COMMANDER_BLAST_CAT"
    ud.selfdestructas = "COMMANDER_BLAST_CAT"
    ud.candgun = true
    ud.weapondefs.disintegrator.damage.commanders = "1"
      end
    end --for
...
for name, ud in pairs(UnitDefs) do
  if (not tobool(ud.cancapture) and tobool(ud.builder) and tobool(ud.canmove) and 
      ud.faction == 'arm' and not ud.yardmap and name ~= "armcarry" and name ~= "armcsa")  then
    ud.cancapture = true
    ud.capturespeed = tonumber(ud.workertime) * 2
  end
end
...
for name, ud in pairs(UnitDefs) do
  if (not tobool(ud.canresurrect) and tobool(ud.builder) and tobool(ud.canmove) and
      ud.faction == 'core' and not ud.yardmap) then
    ud.canresurrect = true
    ud.resurrectspeed = tonumber(ud.workertime) / 1.2
  end
end
...
if (modOptions and tobool(modOptions.chickens)) then 
	newchickens = {
		...
		chicken_spidermonkey = 'newch_spiderspikey2',
		chickenc = 'newch_spider2',
...
--- Base Foundation
--------------------------------------
if (modOptions and tobool(modOptions.foundations)) then 
	Spring.Echo '============test'
    for name, ud in pairs(UnitDefs) do
		if ud.tedclass == [[PLANT]] then
			Spring.Echo ('found?', name)
			ud.buildinggrounddecaldecayspeed = 0.01
			ud.buildinggrounddecalsizex      = 20
			ud.buildinggrounddecalsizey      = 20
			ud.buildinggrounddecaltype       = [[metalfloor.dds]]
			ud.usebuildinggrounddecal        = true
		end
	end
end

--- factory cost
if (modOptions and modOptions.factorycostmult and modOptions.factorycostmult ~= 1) then
  local factorycostmult = modOptions.factorycostmult
  for id, ud in pairs(UnitDefs) do
    if (tobool(ud.builder) and (ud.buildoptions) and (tonumber(ud.maxvelocity) == 0) and ud.unitname ~= "cornanotc" and ud.unitname ~= "armnanotc") then
      if (ud.buildcostenergy and ud.buildcostmetal) then
        ud.buildcostenergy = tonumber(ud.buildcostenergy) * factorycostmult
        ud.buildcostmetal = tonumber(ud.buildcostmetal) * factorycostmult
        ud.buildtime = tonumber(ud.buildtime) * factorycostmult
      end
    end
  end
end
...
--- wreckage
if (modOptions and modOptions.wreckagemult and modOptions.wreckagemult ~= 1) then
  local wreckagemult = modOptions.wreckagemult
  for id, ud in pairs(UnitDefs) do
    local features = ud.featuredefs
    if features then
      for id,feature in pairs(features) do
        feature.metal = tonumber(feature.metal) * wreckagemult
        if feature.reclaimtime ~= nil then
          feature.reclaimtime = tonumber(feature.reclaimtime) * wreckagemult
        end
      end
    end
  end
