...
local function RefreshMetal(x, z, ignore)
  local units = Spring.GetUnitsInCylinder(x, z, radius*2)
  for i=1, #units do 
  ...
  for i=1, #units do 
    ...
    if (UnitDefs[unitDefID].isMetalExtractor and units[i] ~= ignore) then
  ...
  for i=1, #units do
    local unitDefID = spGetUnitDefID(units[i])
    if(UnitDefs[unitDefID].customParams.chickenfac and units[i] ~= ignore) then
      local x, _, z = spGetUnitPosition(units[i])
...
function gadget:UnitDestroyed(unitID, unitDefID, unitTeam)
  if (UnitDefs[unitDefID].customParams.chickenfac or 
      UnitDefs[unitDefID].isMetalExtractor) then
    local x, _, z = spGetUnitPosition(unitID)
