...
local GetUnitRulesParams = Spring.GetUnitRulesParams
...
local function AddCloakShieldRulesParam(unitID, state)
  local params = GetUnitRulesParams(unitID)
  local haveCloakShield = false
  for i,p in ipairs(params) do
    if (next(p) == "cloak_shield") then
      haveCloakShield = true
      break
    end
  end

  if (not haveCloakShield) then
    Spring.CreateUnitRulesParams(unitID, { { ["cloak_shield"] = 0 } })
  end
...
local function UpdateCloakees(data)
  ...
  -- transporting. this does not fix nested transports
    if (UnitDefs[udid].isTransport) then
...
function gadget:GameFrame(frameNum)
  ...
  for unitID, data in pairs(cloakers) do
    if (data.delay) then
      ...
      ShrinkRadius(data)
    elseif (GetUnitIsStunned(unitID)) then
      ShrinkRadius(data)
