...
function Object:New(obj)
  ...
  --if (debug or self.debug) then
  --  Spring.Echo("New object",obj.name)
  --end
...
function Object:Dispose()
  if (not self.disposed) then
    --if (debug or self.debug) then
    --  i = i - 1
    --  Spring.Echo("Disposing object",self.name)
    --end
...
function Object:AddChild(obj, dontUpdate)
  ...
  if (obj.name) then
    if (self.childrenByName[obj.name]) then
      error(("there is already a control with the name `%s`"):format(obj.name))
      return
    end
