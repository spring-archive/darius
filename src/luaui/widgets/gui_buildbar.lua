-- $Id: gui_buildbar.lua 4190 2009-03-27 01:27:59Z carrepairer $
...
-- saved values
local bar_side         = 1     --left:0,right:1,top:2,bottom:3
...
local bar_iconSizeBase = 40    --iconSize o_O
...
local function DrawButton(rect, unitDefID, options)
  ...
  -- draw frame border
  if WG.GetBuildIconFrame then
	DrawTexRect(rect, WG.GetBuildIconFrame(UnitDefs[unitDefID]),options.alpha or 1)
  end
  ...
  if options['repeat'] then
    DrawTexRect({rect[3]-repIcoSize,rect[2],rect[3],rect[2]-repIcoSize}, image_repeat, 0.65)
  end

  -- hover or pressed?
  if (options.hovered_repeat) then
    DrawTexRect({rect[3]-repIcoSize,rect[2],rect[3],rect[2]-repIcoSize}, image_repeat)
    ...
    if i==openedMenu+1 then
      ...
      if (buildQueue ~= nil) then
        ...
        while (buildQueue[n]) do
          ...
          if (count>0) then
			if WG.GetBuildIconFrame then
              DrawTexRect(bopt_rec,WG.GetBuildIconFrame(UnitDefs[unitBuildDefID]),0.55)
			end
...
function widget:MousePress(x, y, button)
  ...
  if pressedFac ~= hoveredFac and hoveredFac > -1 then  -- select clicked fac in game too
    Spring.SelectUnitArray({facs[hoveredFac+1].unitID})
  end 

  if (button == 3 and hoveredBOpt == -1) then return end  -- FIXME: this is hack that disables waypoint modes - it isntead selects factory directly now
...
function widget:GetTooltip(x,y)
  ...
  elseif (hoveredBOpt>=0) then
    if hoveredBOpt>=0 then
      ...
      return "Build Unit: " .. unitDef.humanName .. " - " .. unitDef.tooltip .. "\n" ..
