...
options = {
	order = { 'autoHideChat', 'noColorName',  'hideSpec', 'hideAlly', 'hidePoint', 'hideLabel', 'text_height', 'max_lines', 
...
function widget:Shutdown()
	...
	--spSendCommands({"inputtextgeo default"}) -- not saved to spring's config file
	--spSendCommands({"inputtextgeo 0.26 0.73 0.02 0.028"}) -- does not work due to engine
	Spring.SetConfigString("InputTextGeo", "0.26 0.73 0.02 0.028") -- spring default
end
...
function widget:Update(s)
	...
	if timer > 2 then
		timer = 0
		spSendCommands({string.format("inputtextgeo %f %f 0.02 %f", window_console.x / screen0.width, 1 - (window_console.y + window_console.height) / screen0.height - 0.023, window_console.width / screen0.width)})
	end
end
