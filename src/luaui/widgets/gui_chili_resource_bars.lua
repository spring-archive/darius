...
function widget:Update(s)
	...
	local stallingE = (eCurr < eStor * 0.1) and (eCurr < 1000) and (eCurr > 0)
	...
	if (next_update > os.clock()) then return end
	next_update = os.clock() + 1
	...
	-- update build power bar
	-- TODO optimization?, better algorithm?
	local bp_aval = 0
	local bp_use = 0
	local unitsByDefID = Spring.GetTeamUnitsSorted(GetMyTeamID())
	for id,ud in pairs(builderDefs) do
		local units = unitsByDefID[id]
		if units then
			for i=1,#units do
				bp_aval = bp_aval + ud.buildSpeed
				--local metalMake, metalUse, energyMake,energyUse = Spring.GetUnitResources(units[i])
				--bp_use = bp_use + math.max(math.abs(metalUse), math.abs(energyUse))

				local commands = Spring.GetUnitCommands(units[i])
				if commands and (#commands > 0) then
					bp_use = bp_use + ud.buildSpeed
				end
			end
		end
	end
