function widget:GetInfo()
  return {
    name      = "Context Menu",
    desc      = "v0.02 Chili Context Menu\nPress [Space] and click an unit for detailed informations about it.",
    author    = "CarRepairer",
    date      = "2009-06-02",
...
local function printunitinfo(ud, lang, buttonWidth)	
	...
	local stack1 = StackPanel:New{
		...
		itemPadding = {1,1,1,1},
		itemMargin = {2,2,2,2},
		height = blurbheight+10,
		width = 50,
		resizeItems = false,
		children = icons,
	}
	
	local stack2 = Grid:New{
	...
	return 
		{
			StackPanel:New{
				...
				itemPadding ={2,2,2,2},
				itemMargin = {0,0,0,0},
				height = stack1.height + helptextbox.height + stack2.height +100 ,
				width = '100%',
				resizeItems = false,
				children = { stack1, helptextbox, stack2 },
			},
...
local function MakeUnitContextMenu(unitID,x,y)
	...
	local window_height = (B_HEIGHT+6)*4
	...
	children[#children+1] =  Button:New{ 
		caption = 'Close', 
		--OnMouseUp = { function(self) hideWindow(window_unitcontext) end }, 
		OnMouseUp = { function(self) self.parent.parent:Dispose() end }, 
		width=window_width, 
		backgroundColor=color.sub_back_bg, 
		textColor=color.sub_back_fg,
	}
