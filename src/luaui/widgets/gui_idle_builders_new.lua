...
local function IsIdleBuilder(unitID)
  ...
  if ud.buildSpeed > 0 then  --- can build
    ...
    elseif ud.isFactory then
      ...
      -- don't show factories with less than NEAR_IDLE units in repeat
      -- FIXME: factory set to repeat and "alt"+build something counts as repeat
...
function widget:DrawWorld()
  ...
  if iconNum < 0 then
	...
	if type(unitID) == 'table' then
		unitID = unitID[((Clicks[unitDefID] + 1) % table.getn(unitID)) + 1]
	end
...
function widget:MouseRelease(x, y, button)
  ...
  local unitID = drawTable[iconNum+1].units	
	...
	if type(unitID) == 'table' then
		...
		unitID = unitID[(Clicks[unitDefID] % table.getn(unitID)) + 1]
	end
