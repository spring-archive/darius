...
function widget:GetInfo()
  return {
    name      = "SelectionHalo A",
    desc      = "Shows a halo around selected and hovered units/features plus allied selected units.", -- needs gui_selectionsend.lua
...
function widget:Update(dt)
  ...
  visUnit_a = {}
  visUnitNum_a = 0
  for i=1,#selUnits_a do
    local unitID = selUnits_a[i]
    if (IsUnitVisible(unitID,nil,true)) then
      visUnitNum_a = visUnitNum_a+1
      visUnit_a[visUnitNum_a] = unitID
    end
  end
...
function widget:DrawWorld()
  if (visUnitNum==0)and(visUnitNum_a==0)and(hoveredType=="ground") then
    return;
  end
  ...
  --ALLIED SELECTED UNITS
  if (visUnitNum_a>0) then
    gl.ColorMask(false)
    gl.StencilFunc(GL.ALWAYS, 1, 1)
    gl.StencilOp(GL_KEEP, GL_KEEP, GL_REPLACE)
    gl.PolygonMode(GL.FRONT_AND_BACK,GL.FILL)
    for i=1,visUnitNum_a do
      glUnit(visUnit_a[i],true,-1)
    end
