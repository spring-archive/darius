...
SpringMapEdit is an application for editing the various parts
of which a Spring (http://spring.clan-sy.com/) map consists
within a full 3D environment.
It was written by me in in Java,
and uses SWT and JOGL.
...
-Typemap/Metalmap/Vegetationmap viewing and editing. Toggle to overlay texturemap.
...
-Windows
-Linux   (needs testing. especially if the startup.sh scripts work)
...
Software
-Java (I recommend JRE 1.6 - http://www.java.com)
-SWT 3.4 library (included in this package - http://www.eclipse.org/swt/ )
...
and check some dependencies SWT and JOGL have. (Especially on Linux/OSX (GTK?))
...
Linux:   one of the "start_lin_x86/64.sh" files.
OSX:     someone needs to supply me those files. 
(Basically it should be the same commandline as for windows/linux)

I recently decided to remove the different startup-files for different amount of RAM.
...
If Java can not find a batch of RAM as large as given number, it won't start,
so just set it a little lower than your real RAM.
...
X		Flatten Heightmap to given height (Uses height from "HeightBrush" in "Set" mode)
C		Randomize Heightmap (use Smooth afterwards for something useful)
...
5.1 Why are there multipe start skripts with different numbers (512/768/1024) ?

Those numbers give the maximum heap-space the Java-VM may allocate.
So if you get OutOfMemory Exceptions you may want to try a higher number.
If the Java-VM does crash at startup, try a smaller number again.
You may finetune it by editing the startscripts: look for "-Xmx512m" 
and try some other number.


5.2 Why do the Sliders jump around when selecting a different Brush-Mode?

Each Brush-Mode (each SubMode too) has its own settings,
which are reflected by the Strength+Size Sliders.


5.3. When importing SM2 it complains about missing features?
...
This may be subject to change if more sophisticated import is at place
(import directly from sd7, take care of required files at own...).
...
6.1 How to disable Water when exporting to SM2?
